...
use od_store::{RepoFilter, RepoStore, SortField, SortOrder, SqliteStore, YamlStore};
...
fn init_yaml(path: &std::path::Path) {
    let empty = od_core::CanonicalData::new();
...
fn populate_store(store: &dyn RepoStore) {
    let repos = vec![
        ...
        test_repo("owner/python-repo", "Python", 15000, &["python", "ml", "ai"]),
...
fn test_sqlite_filter_by_topic() {
    ...
    let repos = store.list_repos(&filter).unwrap();
    assert_eq!(repos.len(), 2);
    assert!(repos.iter().all(|r| r.metadata.topics.iter().any(|t| t == "cli")));
}
...
fn test_sqlite_integrity_check() {
    ...
    assert!(store.is_ok(), "SQLite integrity check should pass on fresh DB");
}
