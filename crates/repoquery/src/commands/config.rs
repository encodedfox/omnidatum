...
async fn run_init() -> Result<()> {
    let config_path = OmnidatumConfig::config_path();
...
async fn run_show() -> Result<()> {
    ...
    println!("Config location: {:?}", OmnidatumConfig::config_path());
    Ok(())
}
...
async fn run_set(key: &str, value: &str) -> Result<()> {
    ...
    match key {
        ...
        "storage.mode" => match value {
            "yaml" => config.storage.mode = od_core::config::StorageMode::Yaml,
            "sqlite" => config.storage.mode = od_core::config::StorageMode::Sqlite,
            "dual" => config.storage.mode = od_core::config::StorageMode::Dual,
            _ => return Err(anyhow::anyhow!("Invalid storage mode: {}. Use yaml, sqlite, or dual", value)),
        },
        "credentials.source" => match value {
            "env" => config.credentials.source = od_core::config::CredentialSource::Env,
            "file" => config.credentials.source = od_core::config::CredentialSource::File,
            "keychain" => config.credentials.source = od_core::config::CredentialSource::Keychain,
