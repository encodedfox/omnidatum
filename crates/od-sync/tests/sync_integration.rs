...
/// Helper: build a minimal OmnidatumConfig with a mock token and store path.
fn mock_config() -> OmnidatumConfig {
    let mut config = OmnidatumConfig::default();
    config.credentials.source = od_core::CredentialSource::Env;
...
async fn test_paginated_response() {
    ...
    let _mock1 = server.mock("GET", "/user/starred?per_page=100&page=1")
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_header("Link", "<{url}/user/starred?per_page=100&page=2>; rel=\"next\"")
        .with_body(r#"[{"id":1,"name":"repo1","full_name":"owner/repo1"}]"#)
        .create();

    // Page 2 (last page — no Link header)
    let _mock2 = server.mock("GET", "/user/starred?per_page=100&page=2")
        .with_status(200)
        .with_header("content-type", "application/json")
    ...
    let resp1 = client.get(&page1_url)
        .header("Authorization", "Bearer test_token")
        .send()
        .await
        .unwrap();
    ...
    let resp2 = client.get(&page2_url)
        .header("Authorization", "Bearer test_token")
        .send()
        .await
        .unwrap();
...
fn test_network_error_handling() {
    ...
    let result = rt.block_on(async {
        let client = reqwest::Client::builder()
            ...
            .build()
            .unwrap();
        client.get("https://192.0.2.1:9999/api")
            .timeout(std::time::Duration::from_millis(100))
            .send()
            .await
    });

    assert!(result.is_err(), "Expected network error for unreachable address");
}
